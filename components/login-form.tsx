 ...
 * sign-up flow.
 *
 * On success: navigates to the `next` query param if present, else /protected.
...
export function LoginForm({
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (mode === "password") {
      ...
      if (ok) router.push(next ?? "/protected");
    } else {
      const ok = await magicLinkFlow.submit({ email });
      if (ok) {
        const params = new URLSearchParams({ type: "magiclink", email });
        router.push(`/auth/check-inbox?${params.toString()}`);
      }
    }
  }
