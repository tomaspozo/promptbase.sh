 ...
 *
 * Wires the seven invitation/membership RPCs (membership_list,
 * membership_remove, membership_update_role, invitation_list,
 * invitation_create, invitation_resend, invitation_revoke) into a
 * single page. Permission gating: the route is guarded server-side by
 * app/settings/layout.tsx (requirePermission("membership.read")), and the
 * management controls are gated with useHasPermission(...) — UX only. The
 * backend auth_verify_access() guard in each RPC is the real gate, so a
 * caller without permission gets a 403 even if the UI is bypassed.
 *
 * Uses vanilla useState + useEffect for fetching to avoid pulling
 * @tanstack/react-query into the Next.js scaffold. If you adopt
 * react-query elsewhere, refactor to use it here too — the Vite mirror
 * at routes/_auth/settings/members.tsx already does.
 */
...
function InviteForm({
  ...
  // UI gating (UX only — the backend auth_verify_access guard is the real gate).
  ...
  return (
    <form onSubmit={handle} className="flex flex-col gap-3 sm:flex-row sm:items-end">
      ...
      <Button
        ...
        title={canInvite ? undefined : "You don't have permission to invite members"}
        className="h-11"
      >
...
function InvitationRow({
  ...
  // UI gating (UX only — backend auth_verify_access is the real gate).
